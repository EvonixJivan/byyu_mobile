  ...
  Future<dynamic> newFilters(String? catId) async {
    try {
      ...
      if (response != null && response is DioResult) {
        responseBody = response.data; // unwrap
      }

      // Now ensure responseBody is a Map
      if (responseBody is Map<String, dynamic>) {
        final status = responseBody['status']?.toString() ?? '';
        if (response.statusCode == 200 && status == "1") {
          // Parse into typed model
          final model = NewFilterModel.fromJson(responseBody);
          recordList = model.data; // List<FilterCategory>
          print("Parsed filters count: ${recordList?.length ?? 0}");

          // return model or recordList depending on getDioResult expectation
          return getDioResult(response, model);
        } else {
          recordList = null;
          return getDioResult(response, null);
        }
      } else {
        // unexpected shape — print for debugging
