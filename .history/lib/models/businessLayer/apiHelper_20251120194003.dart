...
//PRODUCTFILTER DELIVERY >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.BELOW
  Future<dynamic> productFiltersDelivery(int catId, int page,
      ProductFilter productFilter, int isSubscription) async {
    try {
      Response response;
      var dio = Dio();
      ...
      print(
          "#############################<<<<<<<<<<<<<<productFiltersDelivery>>>>>>>>>####################################");
      print('${global.nodeBaseUrl}productFiltersDelivery?page=$page');
      print(formData.fields);
      response = await dio.post(
          '${global.nodeBaseUrl}productFiltersDelivery?page=$page',
          data: {
            "user_id":
                global.currentUser.id != null ? global.currentUser.id : "",
          }, // formData,
