...
//
  ...
  Future<dynamic> newFilters(String? catId) async {
    try {
      final dio = Dio();
      ...
      List<NewFilterModel> parsedList = [];
      Map<String, NewFilterModel> parsedMap = {};

      final status = response.data?['status']?.toString() ?? "";
      print("STATUS CHECK NEW FILTER API: ${response.statusCode}");
      print("RAW response.data.runtimeType: ${response.data.runtimeType}");
      print(
          "RAW response.data['data'] runtimeType: ${response.data?['data']?.runtimeType}");
      ...
      //     // Correct: map list elements -> NewFilterModel
      //     parsedList = rawData
      //         .map((x) => NewFilterModel.fromJson(x as Map<String, dynamic>))
      //         .toList();
      //   } else if (rawData is Map) {
      //     // Correct: map entries -> MapEntry(key, NewFilterModel)
      //     parsedMap = Map<String, NewFilterModel>.fromEntries(
      //       rawData.entries.map((e) => MapEntry(
      //             e.key.toString(),
      //             NewFilterModel.fromJson(e.value as Map<String, dynamic>),
      //           )),
      //     );
      //     // Also produce a list form if you want:
      //     parsedList = parsedMap.values.toList();
      //   } else {
      ...
      //   print("Parsed filters count: ${parsedList.length}");
      // } else {
      ...
      return {
        ...
        // Most UIs prefer a list — I return parsedList. Use parsedMap if you need keys.
        "data": response.data,
        "parsedMap": response.data,
        "raw": response.data,
      };
    } catch (e, st) {
  ...
  try {
    ...
    if (response.statusCode == 200 && status == "1") {
      ...
      if (response.data["data"] is List) {
        ...
        print("FILTER CATEGORY API SUCCESS → ${response.data["data"]} items");
      } else {
