...
//
  ...
  newfilters({required String cat_id}) {}
}
...
Future<dynamic> newFilters(
  ...
  try {
    ...
    dynamic recordList;
    print("RECORD LIST NEW FILTER>>>>>>>>>>>>>${response}");
    ...
    if (response.statusCode == 200 && status == "1") {
      ...
      if (response.data is List) {
        recordList = response.data.map((x) => x).toList();
        print("RECORD LIST NEW FILTER>>>>>>>>>>>>>${recordList}");

        print(" NEW FILTER CATEGORY api sucesssssssss>>>>>>>>>>>>>>>>>>>>>>>");
      } else {
        recordList = response.data["data"] ?? response.data;
        print(" NEW FILTER CATEGORY api check error>>>>>>>>>>");
      }
    } else if (response.statusCode == 200 && status == "3") {
      // special case
      recordList = response.data;
