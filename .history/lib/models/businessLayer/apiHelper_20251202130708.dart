  ...
  Future<dynamic> newFilters(String? catId) async {
    try {
      ...
      if (response.statusCode == 200 && status == "1") {
        ...
        print("Parsed filters count: ${response.data?['data']}");

        recordList = List<NewFilterModel>.from(
            response.data["data"].map((x) => NewFilterModel.fromJson(x)));

        print("Parsed filters count: ${recordList.length}");
      } else {
        print("n2----------------------");
