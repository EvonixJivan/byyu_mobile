...
//PRODUCTFILTER DELIVERY >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.BELOW
  Future<Map<String, dynamic>> productfilterExpress(
      {required String userId}) async {
    try {
      Response response;
      var dio = Dio();

      print('${global.nodeBaseUrl}productfilterExpress');

      response = await dio.post('${global.nodeBaseUrl}productfilterExpress',
          data: {
            'user_id': global.currentUser != null ? global.currentUser.id : "",
          },
          options: Options(
    ...
    try {
      ...
      final response = await dio.post(
        '${global.nodeBaseUrl}productfilterExpress',
        data: {"user_id": userId},
        options: Options(headers: await global.getApiHeaders(false)),
      );

      // Debug print entire response data
      print('productfilterExpress response.data: ${response.data}');

      // Defensive checks: response.data might be Map or something else
      if (response.statusCode == 200) {
        final respData = response.data;
        // Accept both int and string status
        final status = respData is Map && respData.containsKey('status')
      ...
      print("Error in productfilterExpress: $e\n$s");
      return {"status": "0", "data": []};
    }
  }
