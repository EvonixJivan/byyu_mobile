...
//PRODUCTFILTER DELIVERY >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.BELOW
  Future<dynamic> productfilterExpress() async {
    try {
      final dio = Dio();

      // Prepare payload (use FormData if you need file uploads; otherwise a plain map is fine)
      final payload = {
        "user_id": global.currentUser?.id?.toString() ?? "",
      };

      // If you prefer FormData (commented):
      // final formData = FormData.fromMap(payload);

      print("URL: ${global.nodeBaseUrl}productFiltersDelivery");
      print("Payload: $payload");

      final response = await dio.post(
        '${global.nodeBaseUrl}productFiltersDelivery',
        data: payload, // or 'formData' if using FormData
        options: Options(
          headers: await global.getApiHeaders(false),
      ...
      // Normalize status to string to handle "1" or 1
      ...
      if (response.statusCode == 200 && status == "1") {
        ...
        if (rawList is List) {
          final recordList = List<Product>.from(
            rawList.map((x) => Product.fromJson(x as Map<String, dynamic>)),
          );

          return getDioResult(response, recordList);
        } else {
          // data was not a List — return empty list or null based on your convention
          print(
              "productFiltersDelivery: expected data as List but got: $rawList");
          return getDioResult(response, <Product>[]);
        }
      } else {
        // non-success status — propagate raw response for debugging
        print('productFiltersDelivery failed: ${response.data}');
        return getDioResult(response, null);
      }
    } catch (e, st) {
      print("Exception - productFiltersDelivery(): $e");
      print(st);
      // Return an error-like result so callers can handle it consistently
      return {
        "statusCode": null,
        "status": "0",
        "message": e.toString(),
...
//
  ...
  Future<dynamic> getEventFilteredProducts(int eventID, int page, String catID,
    ...
    try {
      ...
      print("G1>>>>>>>>>>>>>>>>>>>>>>>>>>${formData}");
      print("G1----event_details---->${formData.fields}");
...
Future<dynamic> oneAPIfirst(Object? object) async {
  try {
    final dio = Dio();
    ...
    final response = await dio.post(
      "${global.nodeBaseUrl}oneapifirst",
      data: body,
      options: Options(
        headers: await global.getApiHeaders(false),
        validateStatus: (status) => status != null && status < 500,
    ...
    // normalize status (string/int both handled)
    final status = response.data?['status']?.toString() ?? "";
    ...
    if (response.statusCode == 200 && status == "1") {
      ...
      if (response.data["events"] is List) {
        ...
        recordList = response.data["data"] ?? response.data;
        print("api check error>>>>>>>>>>");
      }
    } else if (response.statusCode == 200 && status == "3") {
      // special case
      recordList = response.data;
    } else {
      // fallback error
...
Future<dynamic> filltercatgory(Object? object) async {
  try {
    final dio = Dio();
    ...
    final response = await dio.post(
      "${global.nodeBaseUrl}filltercatgory",
      data: body,
      options: Options(
        headers: await global.getApiHeaders(false),
        validateStatus: (status) => status != null && status < 500,
    ...
    if (response.statusCode == 200 && status == "1") {
      ...
      if (response.data is List) {
        recordList = response.data.map((x) => x).toList();
        print(recordList);
        print(" FILTER CATEGORY api sucesssssssss>>>>>>>>>>>>>>>>>>>>>>>");
      } else {
        recordList = response.data["data"] ?? response.data;
        print(" FILTER CATEGORY api check error>>>>>>>>>>");
      }
    } else if (response.statusCode == 200 && status == "3") {
      // special case
      recordList = response.data;
...
Future<dynamic> searchfilters() async {
  try {
    final dio = Dio();

    final response = await dio.get(
      "${global.nodeBaseUrl}searchfilters",
      options: Options(
        headers: await global.getApiHeaders(false),
        validateStatus: (status) => status != null && status < 500,
      ),
    ...
    if (response.statusCode == 200 && status == "1") {
      ...
      if (response.data["data"] is List) {
        recordList = List.from(response.data["data"]);
        print("FILTER CATEGORY API SUCCESS → ${recordList.length} items");
      } else {
        // unexpected format but handle safely
        recordList = response.data["data"] ?? [];
        print("FILTER CATEGORY → Unexpected structure");
      }
    } else if (response.statusCode == 200 && status == "3") {
      // SPECIAL CASE
      recordList = response.data;
    } else {
      print("FILTER CATEGORY → API FAILED!!");
      recordList = null;
    }
...
Future<dynamic> productfilter(Object? object) async {
  try {
    final dio = Dio();
    ...
    final response = await dio.post(
      "${global.nodeBaseUrl}productfilter",
      data: body,
      options: Options(
        headers: await global.getApiHeaders(false),
        validateStatus: (status) => status != null && status < 500,
