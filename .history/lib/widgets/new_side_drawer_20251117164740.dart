                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => SubCategoriesScreen(
                              ...
                              categoryId:
                                  item["event_id"], // must exist in your API
    ...
    try {
      await apiHelper
        ...
        (result) async {
          if (result["status"] == "1") {
            final list =
                result["data"] is List ? result["data"] : result["data"] ?? [];

            celebrateEvents = List.from(list);
            print("Total events: ${filterCategoryItems.length}");

            setState(() {});
            print(
                "API FILTERRRRR CATEGORRRYYYY>>>>>>>>>>>>>>>>>>>>>>${result}");
            ...
            filltercatgory();
            print(
                "API FILTERRRRR CATEGORRRYYYY FAILLLLLLL>>>>>>>>>>>>>>>>>>>>>>${result}");
          }
        },
      );
    } catch (e) {
