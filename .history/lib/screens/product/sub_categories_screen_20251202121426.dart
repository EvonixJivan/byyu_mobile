...
//FILTER EXPANDED UI BELOW
  Widget buildFiltersDrawerExpansion(
    ...
    //   'Price': ['0-499', '500-999', '1000-1999', '2000+'],
    // };
    final Map<String, List<String>> staticOptions = filterOptions.isNotEmpty
        ? filterOptions
        : {
            // fallback only if API not loaded
            'Occasion': [],
            'Delivery Type': [],
            'Color Palette': [],
            'Packaging': [],
  ...
  newfiltersAPICALL(categoryId) async {
    try {
      ...
      if (result != null && result["status"] == "1") {
        final rawData = result["raw"]["data"];
        print(
            "CHECK APPPPPIIIIIII RAW DATAA>>>>>>>>>${selectedCatID}>>>>>>>>>>${rawData}");

        setState(() {
          filterOptions = extractFilterNames(rawData);
        });

        print("FILTER OPTIONS FROM API: $filterOptions");
      }
    } catch (e) {
