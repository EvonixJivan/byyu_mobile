...
class _SubCategoriesScreenState extends BaseRouteState {
  ...
  List<NewFilterModel> filterList = [];
...
//FILTER EXPANDED UI BELOW
  Widget buildFiltersDrawerExpansion(
    ...
    // ----- ASSUMPTIONS -----
    // - `filterList` is a Map<String, dynamic> containing the "data" object from your API.
    // - FilterCategory.fromJson(...) and FilterItem.fromJson(...) exist (from models you already created).
    // - _buildCheckboxTile({category, option, checked, onToggle}) exists and works.
    // - _showMoreDialog(BuildContext, String categoryTitle, List<String> options,
    //      Set<String> selectedSet, void Function(String option) onToggle) exists.
    // -----------------------

    // Build FilterCategory list from incoming map (filterList)
    ...
    final Map<String, List<String>> staticOptions = {
      for (var c in categories)
        (c.title ?? "Unknown"): c.items
            .map((it) => (it.name ?? it.type?.toString() ?? "").toString())
  ...
  newfiltersAPICALL(categoryId) async {
    ...
    try {
      ...
      if (result != null) {
        ...
        _isMoreDataLoaded = false;
        filterList = List<NewFilterModel>.from(result["data"]);
