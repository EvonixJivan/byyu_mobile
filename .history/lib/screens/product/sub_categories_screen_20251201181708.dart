...
class _SubCategoriesScreenState extends BaseRouteState {
  ...
  void initState() {
    super.initState();
    print(
        "new filters API CALLLLLLLL>>>>>>>>>>>>>>>>>>>>${newfiltersAPICALL()}");
    // print(screenHeading);
...
//getFilteredProductsAPICall...G1
  ...
  newfiltersAPICALL() async {
    // showOnlyLoaderDialog();

    try {
      final result = await apiHelper.newfilters(
        cat_id: selectedCatID.toString(),
      );
      List<Product> _tList = [];
      if (result.status == "1") {
        if (result != null) {
          ...
          _tList = result.data;
          print(
              "NEW FILTER appiiiiiiiiiii callll check abhiii>>>>>>>>>${_tList}");
          ...
          setState(() {});
          print(
              "NEW FILTER appiiiiiiiiiii callll check abhiii111111111111111>>>>>>>>>${_tList}");

          // page++;
        }
      } else {
        print(
            "NEW FILTER appiiiiiiiiiii callll check abhiii22222222222>>>>>>>>>${_tList}");
      }

      // hideLoader();
    } catch (e) {
      print("NEW FILTER apiiiiiiiiiii callll check abhiii44444444444>>>>>>>>>");
      // hideLoader();
    }
  }
