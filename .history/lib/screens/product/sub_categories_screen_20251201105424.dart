...
class _SubCategoriesScreenState extends BaseRouteState {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer:
          // buildFiltersDrawer
          buildFiltersDrawerExpansionSafe(
        context,
        productsFound: 5191,
...
//FILTER EXPANDED UI BELOW
  Widget buildFiltersDrawerExpansionSafe(
    BuildContext context, {
    int productsFound = 0,
    required void Function(Map<String, List<String>> selectedFilters) onApply,
  }) {
    ...
    // local state containers (captured by StatefulBuilder)
    final Map<String, Set<String>> selectedOptions = {
      for (var c in categories) c: <String>{}
    };
      ...
      child: Drawer(
        ...
        child: SafeArea(
          child: StatefulBuilder(builder: (context, setState) {
            ...
            void clearAll() {
              setState(() {
                for (var k in selectedOptions.keys) selectedOptions[k]!.clear();
              });
            }

            Widget buildExpansionTileSafe(String category) {
              final options = staticOptions[category] ?? <String>[];
              const double innerMaxHeight = 240.0;

              return ExpansionTile(
                key: PageStorageKey<String>('exp_$category'),
                ...
                children: [
                  // Constrain size and use a scrollable + column (safe)
                  Container(
                    constraints:
                        const BoxConstraints(maxHeight: innerMaxHeight),
                                ...
                                ...options.map((opt) {
                                  ...
                                  return Container(
                                    ...
                                    child: CheckboxListTile(
                                      ...
                                      controlAffinity:
                                          ListTileControlAffinity.leading,
                                      activeColor: Colors.black, // checked fill
                                      checkColor: Colors.white, // tick
                                      contentPadding:
                                          const EdgeInsets.symmetric(
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return buildExpansionTileSafe(cat);
                    },
                  ),
                ),
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () {
                          ...
                          for (var k in selectedOptions.keys)
                            result[k] = selectedOptions[k]!.toList();
                          onApply(result);
                          Navigator.of(context).maybePop();
                        },
                        style: ElevatedButton.styleFrom(
