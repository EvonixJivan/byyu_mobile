...
//getFilteredProductsAPICall...G1
  ...
  newfiltersAPICALL(categoryId) async {
    try {
      print("ABHIIII11111 CALL >>>>>>>>>>>>>>>>>");

      final result = await apiHelper.newFilters(categoryId.toString());

      if (result != null && result["status"] == "1") {
        final rawData = result["raw"]["data"]; // full API data object

        // ⬇️ Extract names only (Occasion, Delivery Type, etc.)
        final extracted = extractFilterNames(rawData);

        print("ABHIIII11111 Extracted Names >>>>>>>>>>>>>>>>> $extracted");

        // Save the extracted results in YOUR variable
        staticOptions = extracted; // <-- this now replaces your static data
  ...
  Map<String, List<String>> extractFilterNames(Map<String, dynamic> data) {
    ...
    data.forEach((key, value) {
      ...
      result[value["title"].toString()] = names;
      // Example: 'Occasion': ['Christmas', 'New Year', ...]
    });
