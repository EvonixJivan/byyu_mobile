...
//FILTER EXPANDED UI BELOW
  ...
  Widget buildFiltersDrawerExpansion2(
    BuildContext context, {
    int productsFound = 0,
    required void Function(Map<String, List<String>> selectedFilters) onApply,
  }) {
    final List<FilterCategory> categories = filterList
        .map<FilterCategory>(
            (v) => FilterCategory.fromJson(v as Map<String, dynamic>))
        .toList();

    // Create a quick Map<title -> List<String> options> (UI-friendly)
    final Map<String, List<String>> staticOptions = {
      for (var c in categories)
        (c.title ?? "Unknown"): c.items!
            .map((it) => (it.name ?? it.id?.toString() ?? "").toString())
            .where((s) => s.isNotEmpty)
            .toList()
    };

    // STATE (by category title)
    final Map<String, Set<String>> selectedOptions = {
        ...
        child: SafeArea(
          child: StatefulBuilder(builder: (context, setState) {
            ...
            Widget buildExpansionTileByCategory(FilterCategory cat) {
              ...
              final options = staticOptions[categoryTitle] ?? <String>[];
              final visibleCount = options.length <= maxVisibleItems
                  ? options.length
                  : maxVisibleItems;
              final hasMore = options.length > maxVisibleItems;
                      ...
                      children: [
                        ...
                        if (hasMore)
                          Padding(
                            ...
                            child: TextButton(
                              onPressed: () {
                                // Use a local adapter so _showMoreDialog gets a single-arg toggle
                                _showMoreDialog(
                                  context,
                                  categoryTitle,
                                  options,
