...
class _SubCategoriesScreenState extends BaseRouteState {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer:
          // buildFiltersDrawer
          buildSafeExpansionTileWithoutScrolling(
        context: context,
        category: 'Occasion',
        options: [
          'Christmas',
        ...
        selectedSet: <String>{}, // your Set<String>
        onToggle: (option) {
          // toggle logic here
        },
      ),
...
//FILTER EXPANDED UI BELOW
  Widget buildSafeExpansionTileWithoutScrolling({
    required BuildContext context,
    required String category,
    required List<String> options,
    required Set<String> selectedSet,
    required void Function(String option) onToggle,
    int maxVisibleItems = 10, // show up to 10 items; change if you want
  }) {
    // explicit typed values to avoid accidental bool->double casts
    const double verticalPadding = 4.0;
    const double horizontalPadding = 8.0;
    const double itemSpacing = 6.0;

    // show only first N items to avoid tall column
    final int visibleCount =
        options.length <= maxVisibleItems ? options.length : maxVisibleItems;
    ...
    return ExpansionTile(
      ...
      children: [
        ...
        // Build a Column with limited number of CheckboxListTile children (no ListView)
        Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            for (int i = 0; i < visibleCount; i++) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  color: selectedSet.contains(options[i])
                      ? Colors.black12
                      : Colors.transparent,
                  borderRadius: BorderRadius.circular(8.0),
                ),
                child: CheckboxListTile(
                  value: selectedSet.contains(options[i]),
                  onChanged: (_) => onToggle(options[i]),
                  title: Text(
                    options[i],
                    style:
            ...
            // If there are more items, show a "View more" button that expands the full list in a dialog
            if (hasMore)
              Padding(
                padding:
                ...
                child: Align(
                  ...
                  child: TextButton(
                    onPressed: () {
                      // show full options in dialog (safe, separate scroll area)
                      showDialog(
                        context: context,
                        builder: (ctx) {
                          return AlertDialog(
                            ...
                            content: SizedBox(
                              ...
                              child: SingleChildScrollView(
                                // here it's safe because dialog provides bounded height
                                child: Column(
                                  children: options.map((opt) {
                                    ...
                                    return Container(
                                      ...
                                      child: CheckboxListTile(
                                        value: checked,
                                        onChanged: (_) {
                                          // update outer selection via onToggle, then rebuild dialog
                                          onToggle(opt);
                                          // Rebuild dialog content by popping and reopening quickly
                                          // (simple approach to refresh state)
                                                    ...
                                                    child: Column(
                                                      children:
                                                        ...
                                                        return CheckboxListTile(
                                                          ...
                                                          onChanged: (_) {
                                                            onToggle(o);
                                                            // keep the dialog open; simple refresh not implemented here
                                                          },
                                                          title: Text(o),
                                                        );
                                                      }).toList(),
      ...
      onExpansionChanged: (bool open) {
        // no-op here; caller may set state if needed
      },
    );
  }
