...
//product filter xpresssssssss code abhishek below
  Future<void> productFilterDelivery() async {
    showOnlyLoaderDialog();
    try {
      // Call the API and get a Map result
      // ------------------------------
      // If your api helper expects a positional String param:
      // final result = await apiHelper.productfilterExpress(global.currentUser?.id?.toString() ?? "");

      // If your api helper expects a named parameter userId:
      ...
      if (result is Map && result["status"] == "1") {
        ...
        // Parse into model list safely
        final List<SubCateeList> _tList = (rawData is List)
            ? rawData
                .map((e) => SubCateeList.fromJson(e as Map<String, dynamic>))
                .toList()
            : <SubCateeList>[];

        // Update your screen state
        _subCategoryList.clear();
        _subCategoryList.addAll(_tList);

        print(
            "PRODUCT DELIVERY EXPRESS SUCCESS: loaded ${_subCategoryList.length} items");
        setState(() {});
      } else {
        print("PRODUCT DELIVERY EXPRESS FAILED: $result");
        // handle error UI or fallback here if needed
      }
    } catch (e, st) {
