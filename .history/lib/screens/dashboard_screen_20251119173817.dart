...
class _DashboardScreenState extends BaseRouteState with WidgetsBindingObserver {
  ...
  Widget _buildFeatureCard({
    ...
    const double cardHeight = 65; // height tuned to match design
                    ...
                    child: Column(children: [
                      ...
                      Container(
                        ...
                        color: ColorConstants
                            .colorPageBackground, // beige background
      ...
      if (isConnected) {
        await apiHelper
          ...
          if (result != null) {
            ...
            //     print(
            //         "G1----->${_homeScreenData!.eventsbanner![i].eventBannerImg}");
            //     print("G1----->${_homeScreenData!.eventsbanner![i].eventName}");
            //     print("G1----->${_homeScreenData!.eventsbanner![i].id}");
            ...
            //       //     _homeScreenData!.events![i].eventBannerImage! +
            //       //     "?width=500&height=500");
            //       bannerEventImageURL.add(
            //           "https://byyu.b-cdn.net/events_img/event_banner_img/28-10-2025/6.png?width=500&height=500");

            //       print(
            //           "G1----bannerEventImageURL->${global.imageBaseUrl + _homeScreenData!.events![i].eventBannerImage! + "?width=500&height=500"}");
            //     }
            //   }

            //   print("G1----bannerEventImageURL->${bannerEventImageURL.length}");
            //   bannerEventImageURL.add(
            //       "https://byyu.b-cdn.net/events_img/event_banner_img/28-10-2025/6.png?width=500&height=500");
            // }
            // make sure bannerEventImageURL is initialized: List<String> bannerEventImageURL = [];
            if (_homeScreenData?.eventsbanner != null &&
                _homeScreenData!.eventsbanner!.isNotEmpty) {
              for (final eb in _homeScreenData!.eventsbanner!) {
                final bannerImg = eb.eventBannerImg; // could be null
                final eventName = eb.eventName;
                final eventId = eb.id;

                // safe debug prints
                print("G1-----> eventBannerImg: ${bannerImg ?? 'null'}");
                print("G1-----> eventName: ${eventName ?? 'null'}");
                print("G1-----> id: ${eventId ?? 'null'}");

                if (bannerImg != null) {
                  final trimmed = bannerImg.trim();

                  // check non-empty and not 'N/A' (case-insensitive)
                  if (trimmed.isNotEmpty && trimmed.toLowerCase() != 'n/a') {
                    // ensure global.imageBaseUrl ends with '/' exactly once
                    final base = (global.imageBaseUrl ?? '').trim();
                    final normalizedBase = base.endsWith('/') ? base : '$base/';
                    // remove any leading slash from trimmed so we don't end up with '//' in URL
                    final path = trimmed.startsWith('/')
                        ? trimmed.substring(1)
                        : trimmed;

                    final fullUrl = '$normalizedBase$path?width=500&height=500';
                    bannerEventImageURL.add(fullUrl);
                    print('G1----bannerEventImageURL added -> $fullUrl');
                  } else {
                    print(
                        'G1---- skipping bannerImg because empty or N/A: "$trimmed"');
                  }
                } else {
                  print('G1---- skipping bannerImg because it is null');
                }
              }

              print("G1----bannerEventImageURL->${bannerEventImageURL.length}");
            } else {
              print('G1---- no event banners available in _homeScreenData');
            }

            bannerEventIconURL.clear();
            if (_homeScreenData?.eventsoccasion?.isNotEmpty ?? false) {
